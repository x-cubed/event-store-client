...
Returns a Buffer containing a GUID that identifies the subscription, for use with unsubcribeStream().
...
* requireMaster - True, if this request must be processed by the master server in the cluster
* onEventAppeared - The callback to be fired for each event that was written to the stream (can be null)
* credentials - The username and password needed to perform the operation on this stream (ICredentials)
* callback  - The callback to be fired once all the events have been retrieved
...
* credentials - The username and password need to perform the operation on this stream (ICredentials)

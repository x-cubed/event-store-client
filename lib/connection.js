...
Connection.prototype.writeEvents = function(streamId, expectedVersion, requireMaster, events, credentials, callback) {
    ...
    for(var i=0; i<events.length; i++) {
        ...
        if (events[i].data) {
            // FIXME: Allow data to be a Buffer object
            // JSON encode the nested objects
            events[i].data_content_type = 1;
            var json = JSON.stringify(events[i].data);
            events[i].data = new Buffer(json);
        } else {
            events[i].data_content_type = 0;
            events[i].data = [];
        }

        events[i].metadata_content_type = 0;
        events[i].metadata = null;
    }

    var writeEvents = new Messages.WriteEvents();
    writeEvents.event_stream_id = streamId;
    writeEvents.expected_version = expectedVersion;
    writeEvents.events = events;
    writeEvents.require_master = requireMaster;
    var data = writeEvents.encode().toBuffer();

    var correlationId = Connection.createGuid();
    this.sendMessage(correlationId, Commands.WriteEvents, credentials, data, function(pkg) {
        switch (pkg.command) {
            case Commands.WriteEventsCompleted:
                var completed = Messages.WriteEventsCompleted.decode(pkg.data);
                callback(completed);
        }
    });
    return correlationId;
    ...
    connection.sendMessage(correlationId, command, credentials, data, function(pkg) {
        switch (pkg.command) {
            ...
            case Commands.ReadAllEventsForwardCompleted:
                ...
                unpackAndNotifyResolvedEvents(readAllCompleted.events, onEventAppeared);
                break;

            case Commands.NotAuthenticated:
                var readAllCompleted = new Messages.ReadAllEventsCompleted();
                readAllCompleted.commit_position = 0;
                readAllCompleted.prepare_position = 0;
                readAllCompleted.events = [];
                readAllCompleted.next_commit_position = 0;
                readAllCompleted.next_prepare_position = 0;
                readAllCompleted.result = 3; // Access Denied
                readAllCompleted.error = "Not authenticated";
                callback(readAllCompleted);
                break;
    ...
    connection.sendMessage(correlationId, command, credentials, data, function(pkg) {
        switch (pkg.command) {
            ...
            case Commands.ReadStreamEventsForwardCompleted:
                ...
                unpackAndNotifyResolvedEvents(readStreamCompleted.events, onEventAppeared);
                break;

            case Commands.NotAuthenticated:
                var readStreamCompleted = new Messages.ReadStreamEventsCompleted();
                readStreamCompleted.events = [];
                readStreamCompleted.result = 5; // Access Denied
                readStreamCompleted.next_event_number = 0;
                readStreamCompleted.last_event_number = 0;
                readStreamCompleted.is_end_of_stream = true;
                readStreamCompleted.last_commit_position = 0;
                readStreamCompleted.error = "Not authenticated";
                callback(readStreamCompleted);
                break;
